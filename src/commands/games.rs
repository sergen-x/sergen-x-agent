...
pub(crate) async fn get_games(
) -> Arc<Mutex<HashMap<String, Arc<Mutex<dyn Installer>>>>> {
        ...
        Arc::new(Mutex::new(HashMap::new()));
    {
        ...
        games.insert(
            "minecraft".to_string(),
            Arc::new(Mutex::new(minecraft::main::Minecraft)),
        );
    }
    games
}
...
pub(crate) async fn get_commands(
) -> Arc<Mutex<HashMap<String, Arc<Mutex<dyn AsyncCommand>>>>> {
        ...
        Arc::new(Mutex::new(HashMap::new()));
    {
        ...
        commandmap.insert(
            "install".to_string(),
            Arc::new(Mutex::new(InstallCommand)),
        );
        commandmap
            .insert("start".to_string(), Arc::new(Mutex::new(StartCommand)));
    }
    commands
}
